...
RegisterNetEvent('esx:setJob', function(job)
    ...
    -- TriggerServerEvent("mdt:server:ClockSystem")
...
RegisterCommand('mdt', function()
    ...
    if not isDead and not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "anim@move_m@prisoner_cuffed", "idle", 3) and not IsPauseMenuActive() then
        if GetJobType(PlayerData.job.name) ~= nil then
            TriggerServerEvent('mdt:server:openMDT')
        end
    else
        ESX.ShowNotification('~r~Can\'t do that!')
    end
end, false)

AddEventHandler('radialmenu:mdt', function()
    local plyPed = PlayerPedId()
    PlayerData = ESX.GetPlayerData()
    if not isDead and not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "anim@move_m@prisoner_cuffed", "idle", 3) and not IsPauseMenuActive() then
        if GetJobType(PlayerData.job.name) ~= nil then
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Welcome, " ..PlayerData.job.grade_label..' '..PlayerData.meta.user.lastname:sub(1,1):upper()..PlayerData.meta.user.lastname:sub(2), location = playerStreetsLocation, fullname = PlayerData.meta.user.firstname..' '..PlayerData.meta.user.lastname, bulletin = bulletin })
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    for i=1,#vehicles do
        ...
        result.vehicles[i]['model'] = GetLabelText(GetDisplayNameFromVehicleModel(vehicle['model']))
    end
    p = nil
    return cb(result)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    ESX.ShowNotification('~g~GPS has been set!')
end)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    for i=1, #result do
        ...
        result[i]['model'] = GetLabelText(GetDisplayNameFromVehicleModel(vehicle.model))
    end
    cb(result)
...
RegisterNUICallback("toggleDuty", function(data, cb)
    print("duty")
    TriggerServerEvent('ak47_serviceblip:refresh')
    LocalPlayer.state:set('job', PlayerData.job.name, true)
    TriggerServerEvent("mdt:server:ToggleDuty")
    -- TriggerServerEvent('QBCore:ToggleDuty')
    -- TriggerServerEvent('mdt:server:ClockSystem')
    cb(true)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        ESX.ShowNotification("~g~You have set your radio frequency to "..radio)
    else
        ESX.ShowNotification("~r~Invalid Station(Please enter a number)")
    end
end)
...
RegisterNetEvent('dispatch:clNotify', function(sNotificationData, sNotificationId)
    while not PlayerData.job do Citizen.Wait(1000) end
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('dispatch:client:removeCallBlip', data.callid)
    cb(true)
end)
